...
vector<double> runHH(vector<double> parameters, int random_seed, double V0, vector<double> I, double dt, double tfin){
  double t_start = 0.0;
  double t_dur = tfin - t_start;
  int size = int(t_dur/dt)+1;
...
vector<double> wrapper(vector<double> parameters, int random_seed, double V0, vector<double> I, double dt, double tfin) {
  vector<double> V_out;
  V_out.resize(int(tfin/dt));

  V_out = runHH(parameters, random_seed, V0, I, dt, tfin);

    return V_out;
}

PYBIND11_MODULE(HHsimulatorWrapper, m) {
